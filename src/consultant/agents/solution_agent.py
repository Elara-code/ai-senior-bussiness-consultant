...
def build_solution_graph(
    *, model: StructuredModel, search: SearchFunction, identity: Identity
    ...
    prompt = files("consultant.agents.prompts").joinpath("solution_system.md").read_text()
    ...
    analyze_node: Any = build_analysis_node(
        model=model,
        output_schema=SolutionDesignOutput,
...
async def run_solution_agent(
    *,
    ...
    result = await build_solution_graph(
