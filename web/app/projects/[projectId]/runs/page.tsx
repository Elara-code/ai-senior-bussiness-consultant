...
export default function RunsPage() {
  ...
  async function start() {
    ...
    if (projectId === "demo") { setMessage("演示项目使用离线任务模式"); setOffline(true); return; }
    ...
    if (!response.ok) { setMessage("当前项目尚未接入后端任务，展示离线时间线"); setOffline(true); return; }
    const run = await response.json(); setRunId(run.id); setMessage("任务已启动，正在接收实时事件");
  }
  return <div className="page project-page"><div className="page-title"><div><span className="eyebrow">LIVE AGENT OPERATIONS</span><h1>Agent 运行中心</h1><p>业务事件可恢复，模型私有思维链不会展示。</p></div><button className="primary" onClick={start}>＋ 启动需求分析</button></div><div className="offline">{message}</div>{runId ? <RunCenter runId={runId} /> : offline ? <div className="panel" style={{ marginTop: 30 }}><h2>离线演示时间线</h2><p>计划创建 → 材料检索 → 需求分析 → 引用质量检查</p></div> : <div className="panel" style={{ marginTop: 30 }}><h2>等待启动</h2><p>启动后将实时显示计划、检索、分析和质量检查步骤。</p></div>}</div>;
}
